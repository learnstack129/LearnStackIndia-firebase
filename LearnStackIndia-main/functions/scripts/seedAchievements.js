// functions/scripts/seedAchievements.js - FIXED VERSION 🔥
...
async function seedAchievements() {
  ...
  try {
    ...
    const existingDocs = await achievementsCollection.listDocuments();
    if (existingDocs.length > 0) {
      console.log(`🗑️ Deleting ${existingDocs.length} old achievement templates...`);
    ...
    achievementTemplates.forEach((ach) => {
      const docRef = achievementsCollection.doc(ach.id);
      batch.set(docRef, {
        ...ach,
