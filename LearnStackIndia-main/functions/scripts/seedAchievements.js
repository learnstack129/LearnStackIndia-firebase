// functions/scripts/seedAchievements.js - MODIFIED for Firestore
...
// 🚨 We need to use the service class to perform the create/validation logic
// NOTE: We don't need to import the Mongoose model here anymore.
const AchievementService = require('../models/Achievement'); 
const ACHIEVEMENTS_COLLECTION = 'achievementTemplates';


// Complete achievement templates to seed (Data structure remains the same)
const achievementTemplates = [
  ...
  // PERFORMANCE ACHIEVEMENTS
  {
    ...
    points: 300,
    criteria: { type: "speed_completion", value: { count: 10, time: 30 } },
    rarity: "rare"
  },
  {
    id: "lightning_fast",
...
async function seedAchievements() {
  ...
  const achievementsCollection = db.collection(ACHIEVEMENTS_COLLECTION);
  
  try {
    // 1. Delete Existing Documents
    const snapshot = await achievementsCollection.listDocuments();
    if (snapshot.length > 0) {
        console.log(`🔄 Clearing ${snapshot.length} existing achievement templates...`);
        const deleteBatch = db.batch();
        snapshot.forEach(doc => {
            deleteBatch.delete(doc);
        });
        await deleteBatch.commit();
        console.log(`🗑️  Deleted ${snapshot.length} existing templates.`);
    } else {
         console.log('📝 No existing achievement templates found.');
    }
    ...
    achievementTemplates.forEach(achievement => {
        ...
        // Use the achievement 'id' as the document ID for easy lookup, 
        // to mimic Mongoose findOneById capability.
        const newDocRef = achievementsCollection.doc(achievement.id); 
        insertBatch.set(newDocRef, achData);
    });

    await insertBatch.commit();
    console.log(`✅ Successfully inserted ${achievementTemplates.length} achievement templates!`);
